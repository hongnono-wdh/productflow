...
**key 按需动态登记（开发中真用到才加、别等 ⑤ 一次登全）**：写某模块真适配器、真正需要某第三方 key 时，就地 `product-key add` 登记它——`--desc` 写成「给用户看的富文本说明」：**为什么需要、去哪个后台申请、什么格式、关键步骤**（多行写清，操作台会渲染多行）。让用户看到的是「现在要接微信支付、需要商户号 mchid（商户平台 → 账户中心 → API 安全 里拿）」而非干巴巴一个变量名。开发中需要哪个就加哪个、随进展变化；已填的模块继续真实对接、缺的按上文「缺 key 判失败 / 占位」处理。**登某第三方服务的 key 要「成套登」**：真正需要接某个服务时，把该服务真实对接所需的**所有必填 key 一次性 `product-key add` 登全**（如阿里云短信要 AccessKey ID / Secret / 签名 SignName / 模板 CODE 四条就一起登四条；SMTP 邮件要 MAIL_USER + MAIL_PASS 就登两条），且同服务的 `--provider` 用**同一个服务商名**（操作台按 provider 成套分组显示、算「这套还缺几个」）。别只登「代码第一次引用到的那一个」——那样用户看不到整套需求、会漏填另一半。
