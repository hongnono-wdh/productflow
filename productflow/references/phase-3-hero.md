...
python3 "$EDIT" --image artifacts/phase-2/refs/<参考1> [--image <参考2>] --prompt "$PROMPT" --size $SIZE --count 4 --model gpt-image-2 --out-dir artifacts/phase-3/heroes

# B) 改某张已生成图（请求带 baseImage，即用户在画布点选某张图 + 在「生成首图对话框」里说怎么改）
python3 "$EDIT" --image artifacts/phase-3/heroes/<baseImage> --prompt "在该图基础上：<用户对话框里的指令>，保持纯 UI 与平台/风格一致" --size $SIZE --count 2 --model gpt-image-2 --out-dir artifacts/phase-3/heroes

# C) 完全没有参考图 → 才退到文生图
python3 "$GEN" --prompt "$PROMPT" --size $SIZE --count 4 --model gpt-image-2 --out-dir artifacts/phase-3/heroes
```
