...
何时读本文件：Phase 5（功能与数据设计）已 done、进入实现阶段时。本阶段把 ④ 设计稿变成**可运行的前端**（脚手架 + 前端页面 / 交互 + 本地预览），并做**视觉还原**（实现截图对着 ④ 设计稿逐项比对到位）。后端实现是下一阶段 ⑦（见 `phase-7-backend.md`）、测试是 ⑧（见 `phase-8-test.md`）。**本阶段不做功能测试**（单元 / 集成 / E2E / 回归归 ⑦⑧）。
...
> 本节「调试期沟通与开调试现场 / ultracode 实现模式」两节是 ⑥ 前端实现、⑦ 后端实现、⑧ 测试**共用的实现纪律**（放在先跑的 ⑥ 这里，⑦⑧ 回读本文件对应章节即可，不重复造）；下面 verification-before-completion / systematic-debugging 三阶段也通用。**功能测试（单元 / 集成 / E2E / 回归）不在本阶段**——它们在 ⑦（后端单元测试）与 ⑧（集成 / E2E / 回归），本阶段只做**前端实现 + 视觉还原比对**。

- **动手前**：严格按 `direction.md` / ④ 设计稿落地（色板、字体、区块顺序照搬，见 Step 2）；关键交互先把行为想清楚。本阶段**不写功能测试**（在 ⑦⑧），本阶段的验收闸是 Step 2 的**视觉还原比对**。
- **说"完成"前**：调用 verification-before-completion skill。**逐页截图真正生成、且对着 ④ 设计稿逐项比对到位**，才更新 step 为 done；没做到的不登记。
...
> **操作台「预览」按钮（`/api/run-action` → 后台 spawn 本动作）**：⑥⑦ 实现面板有一个按平台自适应的「📱构建并在模拟器预览 / 🖥本地运行预览 / 🌐本地预览」按钮（⑧ 测试阶段同样复用这套本地预览做法），用户点它就是要你**只做本节这件事**——把当前已实现的产品按平台构建并起到他屏幕/模拟器上，不重做/推进整个阶段、不标任何 step/phase done、不改产品代码。起好后 `reply` 告诉他怎么看。下面按平台给做法。
...
- **Web 分支**：起 dev server 后，把它弹进用户浏览器让用户实时看，并把 URL 用 `log`/`reply` 告诉用户：
  ...
  ```bash
  # 持久起 dev server（后台常驻，别前台跑——你是后台 agent，前台会阻塞你、你一结束 server 也随之被杀 → 用户 open 的页面连不上）；
  # dev 命令与端口看 package.json / template-choice.md，直接起、别花大量时间查配置
  nohup npm run dev > /tmp/pf-preview.log 2>&1 & disown     # 或 wrangler dev 等，按预设
  # 轮询确认端口 listen（从 /tmp/pf-preview.log 或 lsof 拿到 PORT）后再弹浏览器：
  open "http://localhost:$PORT"            # macOS；Linux 用 xdg-open "http://localhost:$PORT"
  python3 "$SKILL_DIR/scripts/pf_state.py" reply "已起 dev server（后台常驻），你可以开 http://localhost:$PORT 实时看，我这边继续调"
  ```

  这是"额外让用户能看"，**不替代** headless 截图存档（⑥ 用于视觉还原、⑦⑧ 用于测试）——继续照常用 **Python Playwright（chromium headless）** 截图存档；可复跑测试在 ⑦⑧ 测试步另跑。
...
- **Desktop 分支（P-Desktop）**：本机起桌面应用原生窗口，弹到用户屏幕上实时看；agent 照常驱动 + Playwright webview 截图 / 系统截图存档。**仅当有 GUI 的本机会话时**才起原生窗口；远端/无显示环境跳过弹窗，只 `reply` 告知用户如何本地复现（`cargo tauri dev` 或 `npm run start`），**别让命令失败带成 command error**：
  ...
  ```bash
  # Tauri（推荐）：持久起桌面应用开发模式（后台常驻，别前台——前台会阻塞你、你一结束窗口就关）
  nohup cargo tauri dev > /tmp/pf-preview-desktop.log 2>&1 & disown   # 或 npm run tauri dev
...
本阶段（以及 ⑦ 后端实现、⑧ 测试）开发实现**优先用 ultracode（Workflow 多代理编排）**，不要单线程串行把代码从头写到尾：

- **拆任务并行推进（按当前阶段范围拆，别越界）**：把**当前阶段**的构建拆成相互独立、可并行的任务，能并行就并行——
  - **⑥ 前端实现**：并行各页面 / 组件 / 前端模块。**涉后端项目（Web T2/T3，有 HTTP 后端）**：数据用 mock / 本地桩，**不碰后端 / 不建库 / 不写接口**（后端在 ⑦）；**无后端项目（原生本地 iOS/Android/桌面、纯静态）**：⑦ 隐藏 → 本地数据层（SwiftData / Room / SQLite）随界面在本阶段并行建好（原生 App 无独立后端、"前端即全部"，见 scaffold 各平台分支的建库）。**功能测试不在本阶段**（⑦ 后端单元、⑧ 集成 / E2E / 回归）。
  - **⑦ 后端实现（仅涉后端 Web）**：前端已在 ⑥ 建好，按 ⑤ 契约并行实现各后端模块 / 接口 + 数据层（库）+ 后端单元测试。
  - **⑧ 测试**：对 ⑥⑦ 拼装后的真实产物并行做集成 / E2E 旅程 / 回归（无后端项目做前端集成 / E2E）。
  - 有契约依赖（接口契约、`@Model` 结构）的以 Phase 5 单一事实来源对齐，避免并行时各写各的对不上。
- **对抗式验证**：实现完不要自说自话"写完了"——用**独立 agent 复核代码 + 真正跑测试**来确认。用多代理把覆盖做厚（⑦：后端单元测试；⑧：集成 / E2E / 回归四类 + test-report 门禁）。
- **四条分支各按范围拆**：**Web** —— ⑥ 拆各页面前端（mock 数据），⑦ 拆各端点后端 + 建库 + 单元测试，⑧ 拆集成 / E2E / 回归；**iOS / Android / 桌面（无独立后端、⑦ 隐藏）** —— 整个 App 在 ⑥ 并行拆：视图（SwiftUI / Compose / 复用 ④ 的 Web 前端）+ 本地数据层（SwiftData / Room / SQLite），⑧ 再对成品做集成 / E2E / 回归。
...
> **阶段归属**：脚手架（Step 1）+ 前端（Step 2）是本阶段（⑥ 前端实现）的两步活。后端（按 ⑤ 契约实现并对接前端）+ 后端单元测试 + 接口文档在 ⑦ 后端实现阶段做（见 `phase-7-backend.md`）；集成 / E2E 旅程 / 回归 + test-report 门禁在 ⑧ 测试阶段做（见 `phase-8-test.md`）。**无后端项目**（DEC-5 判无后端）⑦ 隐藏，功能测试（前端集成 / E2E）在 ⑧ 做——本阶段一律不做功能测试。
...
**Web 分支（T1/T2/T3）**：搭前端目录 + 后端/Worker **目录骨架**（按档，**scaffold 只建结构、不实现后端逻辑**——后端实现在 ⑦），装依赖。完成标志：依赖装好、dev server 能起、空页面能访问。
...
3. **按组件映射拼、不肉眼临摹**：读 `spec show` 的 `pages[].components`（④ 定的「设计元素→组件」映射）+ `artifacts/phase-2/component-catalog.md`，**用同款组件库组件拼页面**（目录里没有的才定制）；营销页 `type=marketing` 按骨架组件 + `assets[]` 素材嵌入。
   - **照该页 `redline` 精确规格做（还原度方案 A③，别只凭 direction.md 文字）**：`pages[<id>].redline` 里有 `palette`（每档 `token` + 精确 `value` hex，直接用编译出的 CSS 变量、别肉眼调色）、`designWidth`（实现与截图的基准视口）、`icons`（设计图 icon→lucide 组件名，**同位置直接放该矢量图标**）、`typeScale`（字号/行高档）。与文字冲突时以 redline 精确值为准。**还原「质感」不止还原结构**——图标别降级（设计稿是品牌/彩色/写实图标就照做、别简化成单色线框或字母块占位）、卡片/容器补足层次（渐变/阴影/描边，别一片平）、留白照设计稿密度收紧（别拉大间距让页面变空、一屏露的内容变少）。这三点正是下面第 8 轴裁判会重点挑的，实现时就做到位、别等打回。
...
（**涉后端项目**：本阶段只做前端、数据用 mock，**后端在 ⑦、测试在 ⑧**——⑥ 不必等后端、也不在本阶段写后端逻辑或功能测试。**无后端项目**：⑦ 隐藏，本地数据层与界面用 ultracode 并行推进，功能测试在 ⑧。）
...
**iOS 分支（P-iOS）**：界面用 **SwiftUI** 视图实现，按 direction.md 的色板/字体/区块顺序落进各 `Views/` 文件（色值用 `Color(hex:)`/资产目录，字体层级照搬到 `.font(...)`）；产品 UI 性质的界面参照下面"产品 UI"那条选 skill。下面的"落地页交付质量底线"是 Web 营销页专属（移动端导航/SEO meta/裸 href 等是 web 概念），iOS App 不适用——iOS 的质量底线在 ⑥ 的视觉还原比对（Simulator 截图 vs ④ 设计稿）+ ⑧ 测试阶段的 XCUITest 旅程里把关。其余 step 2 内容（截图登记、preview-feedback 圈选反馈）iOS 同样适用，见本步末尾的 iOS 截图说明。

**Android 分支（P-Android）**：界面用 **Jetpack Compose** 实现，按 direction.md 的色板/字体/区块顺序落进各 Composable 文件（色值映射到 `MaterialTheme.colorScheme`/自定义 Color，字体层级照搬到 `TextStyle`/`MaterialTheme.typography`）；产品 UI 性质的界面参照下面"产品 UI"那条选 skill。下面的"落地页交付质量底线"是 Web 营销页专属，Android App 不适用——Android 的质量底线在 ⑥ 的视觉还原比对（Emulator 截图 vs ④ 设计稿）+ ⑧ 测试阶段的 Compose UI Test / Espresso 旅程里把关。其余 step 2 内容（截图登记、preview-feedback 圈选反馈）Android 同样适用，见本步末尾的 Android 截图说明。

**Desktop 分支（P-Desktop）**：界面**复用 ④ 设计的 Web 前端**（HTML/CSS/JS），按 direction.md 的色板/字体/区块顺序落地到 `src/`（或 `frontend/`）下各页面文件；Tauri webview 渲染这套 Web 前端作为桌面应用界面；产品 UI 性质的界面参照下面"产品 UI"那条选 skill。下面的"落地页交付质量底线"是 Web 营销页专属，桌面应用不适用——Desktop 的质量底线在 ⑥ 的视觉还原比对（桌面窗口截图 vs ④ 设计稿）+ ⑧ 测试阶段的 `tauri-driver`/Playwright E2E 旅程里把关。其余 step 2 内容（截图登记、preview-feedback 圈选反馈）Desktop 同样适用，见本步末尾的 Desktop 截图说明。
...
- **移动端导航可用**：导航不能在窄屏直接 `display:none` 就完事——给汉堡菜单/可达的区块跳转（带 `aria-expanded`/`aria-controls`）（⑧ 测试阶段会加一个移动视口 390×844 旅程断言导航可达）。
...
- **没有占位/死链**：CTA、文档、社交链接不能指向 `https://github.com` 这类裸占位或编造的数据（如假 star 数）——要么真实地址，要么"即将上线"页（⑧ E2E 可加一条断言无裸占位 href）。
- **图标用图标库、不用字符/emoji**：功能点/状态/按钮图标用图标库（Web `lucide-react`/`heroicons`、iOS SF Symbols、Android Material Icons），**别用 ▤◎⟳✓ 等字符或 emoji 冒充线性图标**——字符图标的粗细/对齐/风格都对不上设计稿，是还原度差之源。design-spec 组件目录里图标条目已指定库（如 `lucide-react`），照它用。
...
**⚙️ 截图环境确定性（专题 C1 · 视觉对比的最高杠杆——先固定环境，再谈比对）**：跨环境的字体渲染/抗锯齿差异会淹没真实差异，截图前先固定：
- **Web/桌面**：固定 headless chromium 版本；视口 PC 1440 / H5 390；截图前 `await page.evaluate(() => document.fonts.ready)`（等字体就绪）；禁动画（Playwright `animations:'disabled'` + 注入 `*{animation-duration:0s!important;transition-duration:0s!important}`）；mask 动态内容（时间戳/随机头像用固定 seed）。
- **iOS/Android**：固定机型 / AVD + OS 版本 + DPI（同一项目全程用同一台）。
- 设计稿侧与实现截图**同视口/同尺寸**，为下面的比对 / diff 对齐做准备。

**⚠️ 视觉还原比对（前端门禁，最容易被跳过、直接决定"前端效果行不行"）**：实现的界面**不是写完截个图就算 done**——必须对着设计稿**一一比对、迭代到位**，否则还原效果差：

1. 把实现的界面截图（同视口/同尺寸（取该页 `redline.designWidth`——Web 桌面 1440 / 移动 375，与 ④ 提取时同坐标系）；iOS 用对应机型 Simulator；Android 用对应 AVD（Emulator）截图（`adb exec-out screencap -p > x.png`）；Desktop 对运行中的桌面窗口截图——Playwright 截 webview，或系统截图（macOS `screencapture -x preview-desktop.png`））。
...
**🚦 三层闸门（专题 C · 判定「能不能标 done」；替代"像素相似度%硬卡"）**：视觉比对之上，用三层闸门做可审计验收——

**第 1 层 · LLM 视觉裁判（硬 gate；你就是有视觉能力的 Claude，用最挑剔的设计走查视角，别自评宽松）**：**默认在「找差距」而不是「找相同」**——把设计稿当作必须达到的标准，逐条列出**所有肉眼可辨的差异**（尤其下面第 8 轴质感）。**「元素都在、布局对、主色对」绝不等于 pass**——这是最常见的自欺：结构对但质感差（图标降级、卡片一片平、留白偏大），用户一眼就看出「不像」。对每个产品页，`Read` 同时打开【设计稿】+【实现截图】+【第 2 层差异图】+ 读【本页规格】`spec show` 的 `pages[<id>]`，逐条按 checklist 找差异，输出**分档 verdict + 差异清单**（不给百分比——LLM 绝对打分不可靠），写成 `artifacts/phase-6/fidelity-<页名>.json`：
- checklist 8 轴：① 布局结构 ② 主色与配色（对 token）③ 字体层级 ④ 关键组件存在性（spec.components 是否都在）⑤ 间距节奏 ⑥ 文案一致 ⑦ 组件状态（是否只做了 default、漏了声明的 states）⑧ **视觉质感/精致度（最容易被放过、恰恰是用户最在意的「像不像」）**——逐一比「像不像」而非「有没有」：**图标精度**（设计稿的品牌/彩色/写实图标 vs 实现降级成简化单色线框、字母块占位——差一档就是 major）、**卡片与容器层次**（设计稿的渐变/阴影/描边/明暗分层 vs 实现一片平）、**信息密度与留白松紧**（设计稿紧凑丰富，实现却留白偏大、卡片大而空、一屏露的内容更少 = major）、**圆角/阴影/描边等细节质感**。
- JSON schema：
  ```json
  {"page":"pg-1a2b","platform":"PC","verdict":"pass|needfix|block",
   "diffs":[{"axis":"spacing","severity":"minor|major|blocker","desc":"…","region":"上部"}],"round":1}
  ```
- `verdict`：`pass`=**结构与质感都接近到「一个挑剔设计师并排看会认可是同一设计」**（不是「元素齐了」）；`needfix`=有 major——**第 8 轴的质感 major（图标降级成单色/字母块、卡片一片平无层次、留白明显偏大、信息密度不足）一律算 major，必须进自纠**；`block`=有 blocker（缺关键组件/整体错位）。**判 pass 前先自问：把这两张图并排丢给一个挑剔设计师，他会说「这俩是同一个设计」吗？答不上、或要犹豫，就是 needfix，别放过。** 宁可多判一轮 needfix，也别假 pass 自欺（用户会当场看穿）。
- 登记：`artifact 6 artifacts/phase-6/fidelity-<页名>.json --title "还原度裁判（<页名>）"`。

**第 2 层 · 差异图（软证据，不 gate）**：跑 `fidelity_diff.py` 出差异热力图 + 差异比例，喂给第 1 层裁判当证据、并在操作台展示（兜住 LLM 漏看的位移/缺失）：
```bash
python3 "$SKILL_DIR/scripts/fidelity_diff.py" --design artifacts/phase-4/<页设计稿> --impl artifacts/phase-6/<页实现截图> --out artifacts/phase-6/fidelity-<页名>-diff.png
```
...
> 判「能不能 done」的是**第 1 层分档 + 第 3 层断言**；第 2 层只给证据。

**🔁 3 轮自纠闭环（专题 C5 · 带护栏，别做无约束肉眼重来）**：裁判判 `needfix`/`block` 时，按差异清单**针对性改 → 重截 → 重判**，最多 3 轮，带四条护栏：
- **(a) 渲染跑不起来 = 硬失败**：本轮不进对比、记 `block`（不接受「跑不起来但看着对」的产物）。
- **(b) 只接受确有改进的修订**：新一轮差异清单**条数/严重度**未比上一轮下降 → **回退上一版、停**（防越改越差 / reward hacking）。
- **(c) 3 轮硬上限**：之后仍 `needfix`/`block` → 停，进 `inbox` + 转人工圈选兜底（preview-feedback / P6-5，已发版）。
- **(d) 对比用量化信号**：差异比例（第 2 层）+ 裁判分档，不靠肉眼「差不多」。

```
for 轮 in 1..3:
    截图(环境确定) → 三层闸门(第1/2/3层)
    if verdict==pass 且断言全绿: break ✅
    if 渲染失败: 记 block、跳过本轮
    按 diffs 针对性改（只改差异点、不重做整页）→ 重截 → 重判
    if 新 diffs 未比上轮减少/降级: 回退上一版; break   # 护栏 b
# 3 轮后仍不过 → inbox + 人工圈选兜底（不无限自纠）
```
...
**Web 分支截图**：对运行中的页面截图（桌面端约 1440px 宽、移动端约 390px 宽，截全页）。**浏览器工具**：操作台触发的是 headless 后台 agent，没有浏览器 MCP——截图直接用本机已装的 **Python Playwright（chromium headless）** 写脚本（或 webapp-testing / playwright-cli skill），别去 ToolSearch 找 MCP：
...
2. 写本阶段汇总 `artifacts/phase-6/build-summary.md`（一页：技术栈与目录结构、已实现清单【Web=页面清单 / iOS=屏清单 / Android=屏清单 / Desktop=窗口/屏清单】、视觉还原比对结论、已知限制），并登记：

   ```bash
   python3 "$SKILL_DIR/scripts/pf_state.py" artifact 6 artifacts/phase-6/build-summary.md --title "Phase 6 实现汇总"
   ```

3. 确认本阶段各 step（scaffold + frontend）均为 done、预览截图（**按 ④ 页面清单逐页 × 每个选定平台，每张带 `--page-id`/`--platform`，一页不漏**）+ build-summary.md 均已登记。
   ...
   **⚠️ 标 phase 6 done 前先跑「实现覆盖校验」（硬闸，绕不过）**——`phase 6 --status done` 会自动校验 ④ 每个有设计稿的「页×平台」是否都有带 `page-id` 的实现截图，缺页会**拒绝标 done**。先自己跑一遍看缺哪些：
   ...
   python3 "$SKILL_DIR/scripts/pf_state.py" log "Phase 6 前端实现完成：脚手架 + 前端就绪、视觉还原比对到位，待用户确认进入下一阶段"
   ```

4. 在 CLI 向用户汇报：成品预览截图位置、视觉还原比对结论、已知限制，请用户在网页或 CLI 确认后——**涉后端项目**进入 ⑦ 后端实现（见 phase-7-backend.md），**无后端项目**⑦ 隐藏、直接进入 ⑧ 测试（见 phase-8-test.md）。用户此前明确说过"全自动"则不停留，直接进入下一阶段。
