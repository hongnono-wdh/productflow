...
3. **按组件映射拼、不肉眼临摹**：读 `spec show` 的 `pages[].components`（④ 定的「设计元素→组件」映射）+ `artifacts/phase-2/component-catalog.md`，**用同款组件库组件拼页面**（目录里没有的才定制）；营销页 `type=marketing` 按骨架组件 + `assets[]` 素材嵌入。
   - **照该页 `redline` 精确规格做（还原度方案 A③，别只凭 direction.md 文字）**：`pages[<id>].redline` 里有 `palette`（每档 `token` + 精确 `value` hex，直接用编译出的 CSS 变量、别肉眼调色）、`designWidth`（实现与截图的基准视口）、`icons`（设计图 icon→lucide 组件名，**同位置直接放该矢量图标**）、`typeScale`（字号/行高档）。与文字冲突时以 redline 精确值为准。
...
**第 1 层 · LLM 视觉裁判（硬 gate；你就是有视觉能力的 Claude，用最挑剔的设计走查视角，别自评宽松）**：对每个产品页，`Read` 打开【设计稿】+【实现截图】+ 读【本页规格】`spec show` 的 `pages[<id>]`（应有组件/token/states），逐条按 checklist 找差异，输出**分档 verdict + 差异清单**（不给百分比——LLM 绝对打分不可靠），写成 `artifacts/phase-6/fidelity-<页名>.json`：
- checklist 7 轴：① 布局结构 ② 主色与配色（对 token）③ 字体层级 ④ 关键组件存在性（spec.components 是否都在）⑤ 间距节奏 ⑥ 文案一致 ⑦ 组件状态（是否只做了 default、漏了声明的 states）。
- JSON schema：
  ```json
  ...
  ```
- `verdict`：`pass`=达标；`needfix`=有 major，进自纠（专题 C5，五期加）；`block`=有 blocker（缺关键组件/整体错位）。
