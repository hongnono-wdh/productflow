...
1. 读 `request.keywords`（用户/上一步的风格关键词）+ `request.product`（产品名 + 需求），合成搜索词。
   ...
   - **APP 项目优先用真实参考**：真实 App 界面（竞品 App Store / Google Play 上架页里的官方截图）比 Dribbble 设计稿更直观——去竞品商店上架页抓几张真实界面截图存进 `artifacts/phase-2/refs/`，`explore add-ref ... --title "<App名> 真实界面"` 登记进来，和 Dribbble mockup 互补（别只靠 mockup）。用户也可直接给你参考图。
