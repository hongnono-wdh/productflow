...
两层测试，全部真实跑过。Web 主线见下；**iOS 分支见本步末尾的"iOS 测试分支"**（XCTest + XCUITest 在 Simulator 跑），四类测试门禁两条分支共用。（用 ultracode 时，让独立 agent 复核代码并把四类测试分头并行做厚，对抗式确认而非自说自话——见上文「ultracode 实现模式」。）
