...
**项目详情 = 9 阶段步骤条**：顶部一条横向步骤条（①市场调研 ②找参考 ③首图设计 ④页面设计 ⑤功能与数据 ⑥前端实现 ⑦后端实现 ⑧测试 ⑨部署；无后端项目隐藏 ⑦），点某个阶段进入它**各自的视图**，每个阶段在自己的视图里做事：
...
| ⑥ 前端实现 | 面板 | 脚手架、前端页面 / 交互实现（严格还原 ④ 设计的视觉与样式）、本地预览；**不做功能测试**（统一到 ⑧）|
| ⑦ 后端实现 | 面板 | 后端接口 + 数据、**只做单元测试**、接口 / 交付文档（无后端项目隐藏、跳过）|
| ⑧ 测试 | 面板 | 真实拼装后产物的完整验证：集成 + E2E 端到端旅程 + 回归 + 契约测试 + test-report 门禁（无后端项目做前端集成 / E2E）|
...
| ⑥ 前端实现 | `references/phase-6-frontend.md` | 按所选预设脚手架 → 前端页面 / 交互实现（严格还原 ④ 设计的视觉与样式）→ 本地预览。**不做功能测试**（测试统一到 ⑧） |
| ⑦ 后端实现 | `references/phase-7-backend.md` | 后端接口 + 数据实现 → **只做单元测试** → 接口 / 交付文档。**无后端项目本阶段隐藏、整体跳过** |
| ⑧ 测试 | `references/phase-8-test.md` | 真实拼装后产物的完整验证：集成测试 + E2E 端到端旅程 + 回归 + 契约测试 → **test-report 门禁**（无后端项目做前端集成 / E2E） |
| ⑨ 部署上线 | `references/phase-9-deploy.md` | Web：CF Pages（T1/T2 前端）/ Workers+D1（T2 API）/ 单机 Ubuntu（T3）；iOS：archive→导出 .ipa→上传 TestFlight（停在提审前）；Android：bundleRelease→AAB→上传 Google Play 内部测试（停在生产提审前）；PC 桌面应用：`cargo tauri build` 出安装包（.dmg/.msi/.AppImage）→ 签名/公证→可选上架商店（停在提交前）→ 冒烟 → 交付报告 |
...
| Playwright | 强烈建议 | ②找参考抓图、成品预览与 E2E 旅程测试(P8)都靠它（①市场调研已不截图）。装：`pip install playwright && python3 -m playwright install chromium`，或 `npm i -D @playwright/test && npx playwright install chromium` |
...
| deploy-cf-pages | 增强 | 仅 CF 部署用。缺失→按 phase-9 的 wrangler 命令手动部署 |
...
| **部署期命令行工具** | 视路径 | 仅 Phase 9 用，按选定路径才需要：`wrangler`（CF 路径，`npm i -g wrangler` 或 `npx wrangler`）、`docker`（Docker 部署）、`caddy`（路径 C 自定义域名）。端口检查命令分平台：Linux `ss -ltnp`、macOS `lsof -iTCP -sTCP:LISTEN`。better-sqlite3 裸机部署需编译工具链（`apt install -y build-essential python3`），用 Docker 则镜像自带。Phase 9 走某路径前先 `command -v <工具>` 检测，缺失就提示用户装或改用 `npx`，不要硬跑报 command not found |
