...
| ⑥ 开发实现 | `references/phase-6-implement.md` | 按所选预设脚手架 → TDD → 前后端 / 原生 App / 桌面应用 → 冒烟+截图（Web：Playwright / iOS：Simulator / Android：Emulator / 桌面应用：原生窗口）→ 接口/交付文档 |
| ⑦ 部署上线 | `references/phase-7-deploy.md` | Web：CF Pages（T1/T2 前端）/ Workers+D1（T2 API）/ 单机 Ubuntu（T3）；iOS：archive→导出 .ipa→上传 TestFlight（停在提审前）；Android：bundleRelease→AAB→上传 Google Play 内部测试（停在生产提审前）；PC 桌面应用：`cargo tauri build` 出安装包（.dmg/.msi/.AppImage）→ 签名/公证→可选上架商店（停在提交前）→ 冒烟 → 交付报告 |
